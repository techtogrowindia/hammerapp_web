...
import { shopForgotPasswordSchema, formatZodError } from "@/lib/validation";
import { checkRateLimit, AUTH_RATE_LIMIT } from "@/lib/rate-limit";

// POST /api/shop/forgot-password — send a reset OTP (pre-auth).
export async function POST(req: NextRequest) {
  ...
  const parsed = shopForgotPasswordSchema.safeParse(body);
  ...
  try {
    ...
    await issueShopOtp(shop.id, mobile, "FORGOT_PASSWORD");
    return shopOk({ mobile }, "Password reset OTP sent");
  } catch (err) {
    console.error("[shop/forgot-password]", err);

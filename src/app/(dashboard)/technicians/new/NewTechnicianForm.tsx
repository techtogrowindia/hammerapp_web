...
import { UserPlus, Upload, Loader2, CheckCircle2, Download } from "lucide-react";
import {
  registerTechnician,
  importCsv,
  type RegisterResult,
...
export function NewTechnicianForm() {
  ...
  const [regResult, regAction] = useActionState<RegisterResult | undefined, FormData>(
    registerTechnician,
    undefined,
  );
  const [importResult, importAction] = useActionState<ImportResult | undefined, FormData>(
    importCsv,
    undefined,
  );
      ...
      <div className="flex gap-1 bg-slate-100 rounded-lg p-1 w-fit">
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            tab === "single" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500"
          }`}
        >
          Single Technician
        </button>
        <button
          onClick={() => setTab("bulk")}
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
            tab === "bulk" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500"
          }`}
        >
          Bulk CSV Import
        </button>
          ...
          {regResult?.ok ? (
            <div className="text-center py-6">
              ...
              <div className="flex gap-3 justify-center mt-4">
                <button
                  onClick={() => router.push(`/technicians/${regResult.technicianId}/kyc`)}
            ...
            <form action={regAction} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--accent)]"
                />
              </div>
              <div>
              ...
              <SubmitBtn icon={<UserPlus className="h-4 w-4" />} label="Register Technician" busyLabel="Registering..." />
            </form>
          )}
        </div>
      )}
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-6 space-y-4">
          <div className="bg-slate-50 rounded-lg p-4 text-sm text-slate-600">
            ...
            <code className="text-xs block bg-white border border-slate-200 rounded px-2 py-1.5 font-mono">
              name,mobile,email,service_category
            </code>
            <a
              href="data:text/csv;charset=utf-8,name%2Cmobile%2Cemail%2Cservice_category%0ARamesh%20Kumar%2C9876543210%2Cramesh%40example.com%2CElectrical"
              download="technicians_template.csv"
